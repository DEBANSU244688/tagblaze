...
pub async fn reset_db() -> impl IntoResponse {
    ...
    // 🌱 Seed tags, tickets (linked to users), relations
...
/// 🌱 Seeds users and returns inserted records
...
/// 🌱 Seeds tags, tickets (linked to users), and relations
async fn seed_tags_and_tickets(
    ...
    // Tags
    let tags = vec![
        tag::ActiveModel { name: Set("Bug".into()), ..Default::default() },
        tag::ActiveModel { name: Set("Feature".into()), ..Default::default() },
        tag::ActiveModel { name: Set("Urgent".into()), ..Default::default() },
    ];
    ...
    // Tickets — Link to users (rotate ownership)
    let tickets = vec![
        ticket::ActiveModel {
    ...
    let relations = vec![
        ticket_tag::ActiveModel {
            ...
            tag_id: Set(saved_tags[0].id), // Bug
            ..Default::default()
        },
        ticket_tag::ActiveModel {
            ticket_id: Set(saved_tickets[0].id),
            tag_id: Set(saved_tags[2].id), // Urgent
            ..Default::default()
        },
        ticket_tag::ActiveModel {
            ticket_id: Set(saved_tickets[1].id),
            tag_id: Set(saved_tags[1].id), // Feature
            ..Default::default()
        },
    ];
